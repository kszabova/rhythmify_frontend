...
export class HistogramComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.chantService.findByIncipit("Aetern").subscribe(
      (data:(any)) => {
        this.hist_data = data.map(chant => ({"incipit": chant.incipit, "vol_length": chant.volpiano.length}));
        this.createSvg();
  ...
  private drawBars(data: any[]): void {
    ...
    .domain([0, d3.max(data, (d) => d.vol_length)])
    .range([this.height, 0]);
